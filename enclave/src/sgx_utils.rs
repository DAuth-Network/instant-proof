...
pub fn compute_shared_dhkey(
    ...
    handle.open()?;
    let share_k_result = handle.compute_shared_dhkey(
        prv_k,
        pub_k);
...
pub fn seal(plain_binary: &[u8]) -> ([u8;1024],u32) {
    ...
    let seal_len = unsafe {
        sgx_calc_sealed_data_size(mac_len, en_len)
    };
    info(&format!("sealed len is {}", seal_len));
    unsafe {
        sgx_sealed.to_raw_sealed_data_t(
            raw_sealed.as_ptr() as * mut sgx_sealed_data_t, seal_len
        );
    };
    (raw_sealed, seal_len)
}
