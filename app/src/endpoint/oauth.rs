...
use http_req::{
    request::{get, post, Method, RequestBuilder},
    tls,
    uri::Uri,
};
use serde::{Deserialize, Serialize};
use serde_json::{json, to_string, Map, Result, Value};
...
fn http_post_body(url: &str, req: String, content: Option<&str>) -> String {
    let addr = Uri::try_from(url).unwrap();
    let conn_addr = format!("{}:{}", addr.host().unwrap(), addr.corr_port());
    let stream = TcpStream::connect(conn_addr);
    println!("url {:?}", &addr);
    ...
    match stream {
        Ok(r) => {
            ...
            let response = match content {
                Some(c) => RequestBuilder::new(&addr)
                    .method(Method::POST)
                    .body(req.as_bytes())
                    .header("Connection", "Close")
...
fn http_post(url: &str, req: String, content: Option<&str>) -> String {
...
fn http_get(url: &str, header_name: &str, header_v: &str) -> String {
    println!("new http post");
    let mut writer = Vec::new();
...
pub fn github_oauth(client_id: &str, client_secret: &str, code: &str) -> GenericResult<String> {
...
pub fn google_oauth(cid: &str, cse: &str, url: &str, code: &str) -> GenericResult<String> {
